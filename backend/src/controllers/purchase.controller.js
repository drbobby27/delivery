...
export const createPurchase = async  (req,res) => {
    try {
        const { date_purchase, total_value, employee_id, order_id } = req.body
        if( !date_purchase || !total_value || !employee_id || !order_id ){
            return res.status(400).json({error: "Uno o más campos vacios"})
        }
        const createRegister = await Purchase.create({
            date_purchase, total_value, employee_id, order_id 
        })
